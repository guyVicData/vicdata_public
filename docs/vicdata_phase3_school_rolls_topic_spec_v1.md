...
- **Shared taxonomy** — same five labels for school and geography, confirmed deliberately, so "your shape vs your region's shape" is a direct label comparison, not a translation between vocabularies (school shapes vs. classic demography's expansive/constrictive/stationary). **Correction (2026-08-31, narrative-spec diagnostic): this is not how LA/region classification actually works.** A separate, already-shipped decline-tier system (`src/lib/population-trend.ts`: Growing/Stable/Decline/Steep Decline/Severe Decline, ages 5-15, built for the dashboard's Population Trend card) is what LA and region actually get classified into — not the school-level shape taxonomy described here. The two remain deliberately distinct mechanisms with different age bases (5-15 for the tier system, 5-17 for school shapes); a school's own shape and its LA's tier are two separate figures on the same page, not a direct label comparison. See the state-of-school narrative spec v1 §8 for the corrected version of this design.
...
- ~~Adding the boarding mapping to `dfe_school_census.py`'s own dict (items 3, 5 — fix path now known, not yet done).~~ **Correction (2026-08-31, narrative-spec diagnostic): this is done and live, not outstanding.** `src/lib/roll-data.ts`'s `finishSnapshot()` already reads `boarders_total`/`boarders_male`/`boarders_female` from the DfE census breakdown and exposes a real boarding percentage (`RollSnapshot.boarding`), consumed today by the State of the School page's boarding card, the surrounding-schools comparator, and the current-state narrative generator's Topic 6. Confirmed against real current-period data for multiple schools, including a genuine zero-boarders case (The King's School, Worcestershire) verified against two independent sources (GIAS's own `Boarders` flag and the census headcount) rather than assumed to be a mapping gap.

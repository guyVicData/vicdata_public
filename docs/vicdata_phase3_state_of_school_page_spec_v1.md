...
| Surrounding schools — size/shape context | Nearest 10 schools, matched by age band/phase and sector (state/independent); DfE census aggregate over that list. Deliberately simple, no adaptive radius, no member curation — see rolls spec §4 | **Buildable now** for schools DfE census covers; standalone 6th-form/FE colleges likely have a roll-data gap here (see rolls spec §4) even though they should appear as candidates | **Summary/aggregate only on the free tier — never the named list.** Naming the individual schools behind the comparison is exactly the "school in context" paid territory (Comparator Set), not free/context content. A join prompt sits alongside the summary stat ("The 10 schools behind this comparison are visible to verified members."), following the established no-alarm paywall-copy pattern. **Worth verifying against the live page** — check whether the current build already respects this or currently names the schools, which would be a real gap against the tier boundary, not just a copy issue. |

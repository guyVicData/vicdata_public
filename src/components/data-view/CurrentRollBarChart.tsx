...
export default function CurrentRollBarChart({
  points,
}: {
  ...
  return (
    <div className="space-y-1.5">
      {sorted.map((p) => (
        <div key={p.urn} className="flex items-center gap-2" title={`${p.name}: ${p.value.toLocaleString()}`}>
          ...
          <span className="w-12 shrink-0 text-right text-xs tabular-nums text-neutral-500">{p.value.toLocaleString()}</span>
        </div>
      ))}
    </div>
  );

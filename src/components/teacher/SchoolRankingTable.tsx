...
// leading rank), the school, a small sector icon beside the name (state / independent, in
// the shared sector palette), the figure, and the distance from the school itself. The
...
export function SchoolRankingTable({
  ...
  return (
    <table className={`w-full border-collapse tabular-nums ${fullscreen ? "text-sm" : "text-[12px]"}`}>
      ...
      <tbody>
        {sorted.map((r) => {
          const sector = r.independent === null ? null : SECTOR[sectorOf({ independent: r.independent })];
          return (
            <tr
              key={r.key}
            ...
            >
              ...
              <td className="max-w-0 px-1">
                <span className="flex items-center gap-1.5">
                  ...
                  {sector && (
                    <svg width="11" height="11" viewBox="0 0 12 12" className="shrink-0" role="img" aria-label={sector.label}>
                      <title>{sector.label}</title>
                      {/* A small schoolhouse, filled in the sector colour. */}
                      <path d="M6 1 11 4.5V11H1V4.5z" fill={sector.hex} />
